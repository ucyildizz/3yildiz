    ...
    >
      ...
      <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white py-2.5 hidden md:block relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute -bottom-4 left-1/4 w-24 h-24 bg-red-400/10 rounded-full blur-xl" />
        </div>
        ...
        <div className="container mx-auto px-4 flex justify-between items-center text-sm relative">
          <div className="flex items-center gap-6">
            ...
            >
              <span className="w-8 h-8 rounded-full bg-red-500/20 flex items-center justify-center group-hover:bg-red-500/30 transition-colors">
        ...
        <div className="flex items-center justify-between h-20">
              ...
              <div key={link.href} className="relative group">
                {'children' in link && link.children ? (
                    ...
                    >
                      {(link as any).children.map((child: {href: string; label: string}, idx: number) => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className="block px-4 py-2.5 text-gray-700 hover:text-red-600 hover:bg-red-50/50 transition-all duration-200 hover:pl-6"
                  ...
                  >
                    ...
                    {/* Underline animation */}
                    <span className={cn(
                      'absolute bottom-1 left-4 right-4 h-0.5 bg-gradient-to-r from-red-500 to-red-400 rounded-full transform origin-left transition-transform duration-300',
          ...
          <div className="flex items-center gap-3">
            <Link href="/teklif" className="relative group">
              ...
              {/* Mobile cart icon */}
              <button className="sm:hidden relative p-2 text-gray-700 hover:text-red-600 transition-colors">
                <ShoppingCart className="w-6 h-6" />
                {itemCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-bold animate-bounceIn">
                    {itemCount}
                  </span>
                )}
              </button>
              ...
              <React.Fragment key={link.href}>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive(link.href)
                      ? 'text-red-600 bg-red-50'
                      : 'text-gray-700 hover:text-red-600 hover:bg-red-50/50 hover:pl-6'
                  )}
                  ...
                  <div className="pl-4 space-y-1">
                    {(link as any).children.map((child: {href: string; label: string}) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="block px-4 py-2 text-sm text-gray-600 hover:text-red-600 hover:pl-6 transition-all rounded-lg"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        {child.label}
                      </Link>
